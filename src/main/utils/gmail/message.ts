...
export function parseGmailMessage(accountId: string, message: gmail_v1.Schema$Message): NewGmailMessageRecord {
  ...
  const bodyText = findMimePart(message.payload, 'text/plain')
    || (message.payload?.mimeType === 'text/plain' ? decodeBase64Url(message.payload?.body?.data) : '')
...
export function toStoredMessage(record: {
  ...
  return {
    ...
    timestamp: toTimestamp(record.internalDate),
...
export function buildRawEmail(payload: SendEmailPayload) {
  ...
  if (hasAttachments) {
    const mixedBoundary = `bubbles-mixed-${crypto.randomUUID()}`
    lines.push(`Content-Type: multipart/mixed; boundary="${mixedBoundary}"`, '')
    lines.push(`--${mixedBoundary}`)

    // Create alternative block for body text & HTML
    const altBoundary = `bubbles-alt-${crypto.randomUUID()}`
    lines.push(`Content-Type: multipart/alternative; boundary="${altBoundary}"`, '')
    
    // Plain Text Body
    lines.push(`--${altBoundary}`)
    lines.push('Content-Type: text/plain; charset="UTF-8"', 'Content-Transfer-Encoding: 7bit', '', payload.bodyText || '')

    // HTML Body (if present)
    if (payload.bodyHtml) {
      lines.push(`--${altBoundary}`)
      lines.push('Content-Type: text/html; charset="UTF-8"', 'Content-Transfer-Encoding: 7bit', '', payload.bodyHtml)
    }
    lines.push(`--${altBoundary}--`)
    ...
    for (const attachment of payload.attachments!) {
      lines.push(`--${mixedBoundary}`)
      lines.push(
        `Content-Type: ${attachment.type || 'application/octet-stream'}; name="${sanitizeHeader(attachment.name)}"`,
        `Content-Disposition: attachment; filename="${sanitizeHeader(attachment.name)}"`,
  ...
  // Fallback for emails without attachments
